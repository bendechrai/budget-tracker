...
- **User actions**:
  - **Accept**: creates an income source or obligation with the detected parameters. The next due date (obligations) or next expected date (income) is computed from the suggestion's linked transaction history rather than defaulting to today/null. Two strategies are used depending on day-of-month consistency: if transactions cluster tightly (range ≤ 7 days), a median anchor day determines the next occurrence; if they drift (range > 7), the system advances from the most recent transaction by the detected interval. For twice-monthly patterns, each cluster is checked independently. User overrides still take precedence. For irregular patterns (no detected interval), the system computes a conservative baseline from transaction history (average/median per-period amount, minimum as floor) and names it "Vendor (irregular baseline)". Accepting an expense suggestion also auto-links existing unlinked debit transactions to the new obligation via vendor name similarity
